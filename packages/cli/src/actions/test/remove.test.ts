...
describe('removeAction', () => {
  describe('happy path', () => {
    it('removes a rule from disk, manifest, and lockfile', async () => {
      await seedRule({
        slug: 'pr-create',
        ref: 'rule/org443/pr-create',
        version: 'latest',
      });
      ...
      expect(lockfile.deps?.rules ?? {}).not.toHaveProperty('pr-create');
    ...
    it('removes a rule addressed by its org/slug ref', async () => {
    ...
    it('removes a ref-keyed setup bundle by its full ref', async () => {
      const aipkgRef = 'aipkg://setup/superpowers/Superpowers@1.0.0';
      await writeTestFile(
        JSON.stringify({
          type: 'box',
          ref: 'test/app',
          version: '0.0.0',
          deps: { setups: { 'superpowers/Superpowers': aipkgRef } },
        }),
        'aipkg.json',
      );
      await writeTestFile(
        JSON.stringify({
          deps: { setups: { 'superpowers/Superpowers': { aipkgRef, version: '1.0.0', sha: 'sha256:x' } } },
        }),
        'aipkg.lock',
      );
      await writeTestFile(
      ...
      await writeTestFile('#!/bin/sh\n', '.claude', 'scripts', 'superpowers', 'Superpowers', 'scripts', 'run.sh');

      await removeAction({ type: 'setup', ref: 'superpowers/Superpowers' });

      expect(testFileExists('.claude', 'scripts', 'superpowers', 'Superpowers')).toBe(false);
      const manifest = await readTestJson('aipkg.json');
      expect(manifest.deps?.setups ?? {}).not.toHaveProperty('superpowers/Superpowers');
      const lockfile = await readTestJson('aipkg.lock');
      expect(lockfile.deps?.setups ?? {}).not.toHaveProperty('superpowers/Superpowers');
      const settings = await readTestJson('.claude', 'settings.local.json');
      expect(settings.hooks).toBeUndefined();
    });

    it('clears the statusLine from settings and lockfile when the owning setup is removed', async () => {
      const aipkgRef = 'aipkg://setup/org443/lint@1.0.0';
      await writeTestFile(
        JSON.stringify({
          type: 'box',
          ref: 'test/app',
          version: '0.0.0',
          deps: { setups: { 'org443/lint': aipkgRef } },
        }),
        'aipkg.json',
      );
      await writeTestFile(
        JSON.stringify({
          deps: {
            setups: { 'org443/lint': { aipkgRef, version: '1.0.0', sha: 'sha256:x' } },
      ...
      await removeAction({ type: 'setup', ref: 'org443/lint' });
  ...
  describe('box', () => {
    it('removes every child the box placed plus its own manifest/lockfile entries', async () => {
      ...
      await writeTestFile(
        JSON.stringify({
          deps: {
            ...
            rules: { greet: { aipkgRef: boxRef, version: '1.0.0', sha: 'sha256:box', parent: boxRef } },
            skills: { helper: { aipkgRef: boxRef, version: '1.0.0', sha: 'sha256:box', parent: boxRef } },
          },
        }),
        'aipkg.lock',
      );
      await writeTestFile('# greet', '.claude', 'rules', 'greet.md');
      ...
      expect(testFileExists('.claude', 'rules', 'greet.md')).toBe(false);
      ...
      expect(lockfile.deps?.rules ?? {}).not.toHaveProperty('greet');
