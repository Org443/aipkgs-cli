...
beforeEach(() => setupTestCwd({ prefix: 'aipkg-claude-setup-' }));
afterEach(teardownTestCwd);

describe('installSetup', () => {
  it('namespaces script assets under .claude/scripts/<org>/<slug>/', async () => {
    ...
    expect(await readTestFile('.claude/scripts/acme/lint/scripts/lint.sh')).toBe('#!/bin/sh\n');
    expect(written).toEqual(
      expect.arrayContaining([expect.stringMatching(/\.claude\/scripts\/acme\/lint\/scripts\/lint\.sh$/)]),
    );
  });
  ...
  it('places scripts marked executable with the execute bit set', async () => {
    ...
    expect(await testFileMode('.claude/scripts/acme/lint/scripts/lint.sh')).toBe(0o755);
    expect(await testFileMode('.claude/scripts/acme/lint/scripts/config.json')).toBe(0o644);
  });
  ...
  it('keys the on-disk directory by the full org/key/slug ref', async () => {
    ...
    expect(testFileExists('.claude/scripts/superpowers/team/lint/scripts/lint.sh')).toBe(true);
  });
