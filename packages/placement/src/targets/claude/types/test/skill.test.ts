...
beforeEach(() => setupTestCwd({ prefix: 'aipkg-claude-skill-' }));
afterEach(teardownTestCwd);

describe('installSkill', () => {
  it('writes SKILL.md and nested assets under .claude/skills/<slug>/, preserving the tree', async () => {
    ...
    expect(await readTestFile('.claude/skills/pr-helper/references/guide.md')).toBe('# guide');
    expect(written).toEqual(
      expect.arrayContaining([
        expect.stringMatching(/\.claude\/skills\/pr-helper\/SKILL\.md$/),
  ...
  it('prunes cruft and does not place the aipkg.json manifest inside the skill', async () => {
    ...
    expect(testFileExists('.claude/skills/pr-helper/SKILL.md')).toBe(true);
    expect(testFileExists('.claude/skills/pr-helper/.DS_Store')).toBe(false);
    expect(testFileExists('.claude/skills/pr-helper/node_modules')).toBe(false);
    // The manifest is package metadata, not a skill asset — it must not be placed.
    expect(testFileExists('.claude/skills/pr-helper/aipkg.json')).toBe(false);
  });
});
