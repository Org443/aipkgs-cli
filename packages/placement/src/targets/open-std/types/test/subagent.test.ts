...
beforeEach(() => setupTestCwd({ prefix: 'aipkg-open-std-subagent-' }));
afterEach(teardownTestCwd);

describe('installSubagent', () => {
  it('places the subagent markdown at .agents/agents/<slug>.md', async () => {
  ...
  it('keeps the agents directory flat — only the agent markdown, no manifest or pruned cruft', async () => {
    ...
    expect(testFileExists('.agents/agents/aipkg.json')).toBe(false);
    expect(testFileExists('.agents/agents/.DS_Store')).toBe(false);
  });
});
