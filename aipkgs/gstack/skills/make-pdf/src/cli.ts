...
function printUsage(): void {
  const lines = ['make-pdf — turn markdown into publication-quality PDFs', '', 'Usage:'];
...
function generateOptionsFromFlags(parsed: ParsedArgs): GenerateOptions {
  ...
  return {
    ...
    headerTemplate: typeof f['header-template'] === 'string' ? f['header-template'] : undefined,
    footerTemplate: typeof f['footer-template'] === 'string' ? f['footer-template'] : undefined,
    ...
    switch (parsed.command) {
      case 'version': {
        ...
        try {
          ...
          const versionFile = path.resolve(path.dirname(process.argv[1] || ''), '../../VERSION');
