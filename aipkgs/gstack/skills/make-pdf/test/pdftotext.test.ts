...
const REAL_EXE: string =
  process.platform === 'win32' ? path.join(process.env.SystemRoot ?? 'C:\\Windows', 'System32', 'cmd.exe') : '/bin/sh';
...
describe('resolvePdftotext (override resolution, v1.24-aligned)', () => {
  ...
  test('honors PDFTOTEXT_BIN as a back-compat alias', () => {
    const info = withEnv({ GSTACK_PDFTOTEXT_BIN: undefined, PDFTOTEXT_BIN: REAL_EXE }, () => resolvePdftotext());
    expect(info.bin).toBe(REAL_EXE);
  });

  test('GSTACK_PDFTOTEXT_BIN takes precedence over PDFTOTEXT_BIN', () => {
    const info = withEnv({ GSTACK_PDFTOTEXT_BIN: REAL_EXE, PDFTOTEXT_BIN: '/nonexistent/legacy' }, () =>
      resolvePdftotext(),
    );
    expect(info.bin).toBe(REAL_EXE);
  });
