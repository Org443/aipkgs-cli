...
describe('server.ts — flushBuffers tracker declarations', () => {
  test('every `last*Flushed` tracker referenced inside flushBuffers is declared at module scope', () => {
    ...
    const fnMatch = SERVER_TS.match(/async function flushBuffers\([^)]*\)[^{]*\{([\s\S]*?)\n\}/);
    ...
    expect(trackers.length, 'flushBuffers should reference at least one last*Flushed tracker').toBeGreaterThan(0);
    ...
    for (const tracker of trackers) {
      ...
      // string literals.
      const declared = new RegExp(`(?:^|\\n)let\\s+${tracker}\\s*=\\s*0\\s*;`).test(SERVER_TS);
      expect(
        declared,
