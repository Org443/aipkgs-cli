...
describe('sidepanel security DOM', () => {
  ...
  test.skipIf(!CHROMIUM_AVAILABLE)(
    ...
    async () => {
      ...
      const displayed = await page.$eval('#security-banner', (el) => window.getComputedStyle(el).display !== 'none');
      ...
      const layers = await page.$$eval('.security-banner-layer', (els) => els.map((el) => el.textContent));
  ...
  test.skipIf(!CHROMIUM_AVAILABLE)(
    ...
    async () => {
      ...
      // Initially collapsed
      const initialAria = await page.$eval('#security-banner-expand', (el) => el.getAttribute('aria-expanded'));
      expect(initialAria).toBe('false');
      const initialHidden = await page.$eval('#security-banner-details', (el) => (el as HTMLElement).hidden);
      expect(initialHidden).toBe(true);

      // Click expand
      await page.click('#security-banner-expand');
      const expandedAria = await page.$eval('#security-banner-expand', (el) => el.getAttribute('aria-expanded'));
      expect(expandedAria).toBe('true');
      const expandedHidden = await page.$eval('#security-banner-details', (el) => (el as HTMLElement).hidden);
  ...
  test.skipIf(!CHROMIUM_AVAILABLE)(
    ...
    async () => {
      ...
      const stillVisible = await page.$eval('#security-banner', (el) => window.getComputedStyle(el).display !== 'none');
